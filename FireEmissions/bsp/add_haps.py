...
def main():
    ...
    cons = get_cons(df)
    states = get_states('/proj/ie/proj/GSA-EMBER/BlueSky/ember_2023/haps/flat_costcy_30jan2019_v24.csv')
    print(len(df))
    # Read in the regionalized HAP factors (lb/ton consumed)
    haps =  pd.read_csv('/proj/ie/proj/GSA-EMBER/BlueSky/ember_2023/haps/hapfactors_2014.csv', dtype={'poll': str})
    # Read in the phased lead factors (mg Pb/kg consumed) by state and phase
    lead = get_lead('lead_factors_2020.txt')
    cons = pd.merge(cons, states, on='stfips', how='left')
    print(cons.head())
    # Fix for SD county change
    cons.loc[cons['fips'] == '46102', 'state'] = 'SD'
    # Calc the HAP emis
    haps = pd.merge(cons, haps, on=['type','state'], how='left')
...
def get_states(fn):
    '''
    Get the region to state conv
    '''
    states = pd.read_csv(fn, usecols=['stfips','stabbr'], dtype={'stfips': str})
    states.rename(columns={'stabbr': 'state'}, inplace=True)
